...
This companion note has two goals. First, it rewrites the move-time objects of \cite{Petersen2025} in standard value-function language without changing the underlying microprice model. Second, it adds exact event-time forecasting targets and online learning rules that use the same price-move episodes. Let
\[
...
\]
Then classical microprice is already a move-time relative value function:
\[
g=G_1-\mu\,\1+Bg,
\qquad
\pi^\top g=0.
...
\]
Thus the reinforcement-learning extension is not external to microprice; it is the event-time-discounted version of the same operator calculus. We reserve $\gamma$ for event-time discounting here; the move-time resolvent $(I-\beta B)^{-1}G_1$ studied in Paper~I discounts only across price moves after the reduction. For deterministic horizons,
\[
...
\mu_P=\pi_P^\top r.
\]
The event-time differential value $g_P$ is therefore the exact event-time counterpart of the move-time boundary object from \cite{Petersen2025}.
...
The main paper \cite{Petersen2025} identifies classical microprice as a centered Poisson corrector on the move-time chain. This note is Paper~II in the same sequence. It keeps the move-time reduction and the notation of Paper~I fixed, and asks two Bellman questions: what are the exact event-time forecasting targets on the full chain $P$, and how do those targets translate into online learning rules on the same price-move episodes used by classical microprice?

To keep the two papers aligned, we use the same core dictionary throughout:
\[
r(x):=\E[p_{n+1}-p_n\mid X_n=x],
\qquad
N:=(I-Q)^{-1},
...
G^*:=(I-B+\PiB)^{-1}G_1=g+\mu\1.
\]
Here $g$ is the centered move-time relative value and $G^*$ is the uncentered long-run move-time microprice level. In the centered regimes emphasized in Paper~I, $\mu=0$ and $G^*=g$.

The division of labor is therefore simple. Paper~I proves the move-time Poisson representation, studies the move-time regularizer $(I-\beta B)^{-1}G_1$, and documents the horizon effect and martingale diagnostic. The present note adds exact event-time Bellman values $V^{(H)}$ and $V_\gamma$, the event-time differential value $g_P$, and the corresponding TD/SMDP learning rules. Read together, the two notes show that microprice, Bellman forecasting, and semi-Markov RL are different views of the same reduced operator calculus.
...
\end{equation}
The factor $\gamma^{D_k-1}$ on $\Delta P_k$ appears because the price change is realized on the last event of the episode, exactly as in Proposition~\ref{prop:discounted_move_factorization}. The corresponding tabular SMDP-TD(0) update is
...
The reinforcement-learning extension of microprice is best understood as Paper~II of the same operator story, not as a separate theory. Paper~I reduces event time to move time and identifies $G^*$ as the long-run move-time boundary object. The present note keeps that reduction fixed and adds the exact event-time Bellman objectives $V^{(H)}$, $V_\gamma$, and $g_P$.

This viewpoint keeps the mathematics and the implementation aligned. The exact fixed-horizon Bellman value $V^{(H)}$ matches the forecasting objective, the discounted value $V_\gamma$ gives the cleanest one-shot solve, the event-time differential value $g_P$ is the correct event-time boundary object, the move-time differential recursion learns the classical microprice correction itself, and the move-time SMDP recursion learns the horizon-aware discounted predictor on the same price-move episodes used in \cite{Petersen2025}. The empirical picture is correspondingly simple: fixed-horizon Bellman values produce the largest forecasting gains, while move-time RL updates deliver the largest ergonomic gain.
...
(I-B+\PiB)g=G_1-\mu\1.
\]
Because $\PiB g=\1\pi^\top g$, the normalization $\pi^\top g=0$ reduces this to
\[
(I-B)g=G_1-\mu\1,
\]
which rearranges to \eqref{eq:move_time_average_reward_bellman}. Conversely, if \eqref{eq:move_time_average_reward_bellman} holds together with $\pi^\top g=0$, then applying $\pi^\top$ gives $\mu=\pi^\top G_1$, and adding $\PiB g=0$ recovers \eqref{eq:move_time_relative_value}.
\end{proof}
...
(I-\gamma P)^{-1}\PiP r
=
\sum_{k=0}^{\infty}\gamma^k\PiP r
=
\frac{\mu_P}{1-\gamma}\,\1.
\]
This gives \eqref{eq:discounted_split_event_time}. On the centered subspace $(I-\PiP)\R^L$, the powers of $P$ converge to zero, so the Neumann series converges to $(I-P+\PiP)^{-1}(I-\PiP)r=g_P$, which yields \eqref{eq:discounted_boundary_event_time}.
\end{proof}
